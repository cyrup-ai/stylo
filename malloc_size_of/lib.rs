...
impl MallocSizeOfOps {
    ...
    pub unsafe fn malloc_enclosing_size_of<T>(&self, ptr: *const T) -> usize {
        ...
        match self.enclosing_size_of_op {
            Some(op) => unsafe { op(ptr as *const c_void) },
            None => {
                eprintln!(
                    "BUG: malloc_enclosing_size_of called without available enclosing_size_of_op"
                );
                0
            },
        }
    ...
    pub fn have_seen_ptr<T>(&mut self, ptr: *const T) -> bool {
        match self.have_seen_ptr_op.as_mut() {
            Some(have_seen_ptr_op) => have_seen_ptr_op(ptr as *const c_void),
            None => {
                eprintln!(
                    "Warning: have_seen_ptr called without available have_seen_ptr_op, returning false"
                );
                false
            },
        }
    ...
    B::Owned: MallocSizeOf,
{
    fn size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        match self {
            ...
            std::borrow::Cow::Owned(b) => b.size_of(ops),
        }
    }
}
...
impl<T, const N: usize> MallocShallowSizeOf for smallvec::SmallVec<T, N> {
    fn shallow_size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        if self.spilled() {
            unsafe { ops.malloc_size_of(self.as_ptr()) }
        } else {
...
impl<T, const N: usize> MallocSizeOf for smallvec::SmallVec<T, N>
where
    T: MallocSizeOf,
{
    fn size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
...
impl<T: MallocSizeOf> MallocSizeOf for std::sync::Mutex<T> {
    fn size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        match self.lock() {
            Ok(guard) => (*guard).size_of(ops),
            Err(poisoned) => {
                eprintln!("Warning: Mutex was poisoned, measuring recovered data");
                (**poisoned.get_ref()).size_of(ops)
            },
        }
    }
}
...
impl MallocSizeOf for selectors::parser::AncestorHashes {
    fn size_of(&self, ops: &mut MallocSizeOfOps) -> usize {
        let selectors::parser::AncestorHashes { packed_hashes } = self;
