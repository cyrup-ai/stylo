...
impl<Impl: SelectorImpl> SelectorList<Impl> {
    ...
    pub fn from_iter(mut iter: impl ExactSizeIterator<Item = Selector<Impl>>) -> Self {
        if iter.len() == 1 {
            match iter.next() {
                Some(selector) => Self::from_one(selector),
                None => {
                    // This should never happen with ExactSizeIterator len() == 1
                    panic!("BUG: ExactSizeIterator reported len() == 1 but next() returned None");
                },
            }
        } else {
    ...
    pub fn iter_skip_relative_selector_anchor(&self) -> SelectorIter<'_, Impl> {
        if cfg!(debug_assertions) {
            ...
            match selector_iter.next() {
                ...
                Some(other) => panic!(
                    "BUG: Relative selector does not start with RelativeSelectorAnchor, found: {:?}",
                    other
                ),
                None => panic!("BUG: Relative selector iterator is empty"),
            }
            match selector_iter.next() {
                Some(component) if component.is_combinator() => {},
                Some(other) => panic!(
                    "BUG: Relative combinator does not exist, found: {:?}",
                    other
                ),
                None => panic!("BUG: Relative selector missing combinator"),
            }
        }
        ...
        for compound in compound_selectors {
            ...
            if matches!(
                ...
                match combinators.next() {
                    Some(combinator) => combinator.to_css_relative(dest)?,
                    None => {
                        eprintln!(
                            "Warning: Expected combinator after RelativeSelectorAnchor/ImplicitScope but none found"
                        );
                        // Continue processing without error - this may be a malformed selector
                    },
                }
                continue;
            }
        ...
        W: fmt::Write,
    {
        ...
        match *self {
            ...
            Has(ref list) => {
                dest.write_str(":has(")?;
                let mut first = true;
                for RelativeSelector { selector, .. } in list.iter() {
                    if !first {
                        dest.write_str(", ")?;
                    }
                    first = false;
...
pub mod tests {
    ...
    use crate::parser;
    use cssparser::{Parser as CssParser, ParserInput, ToCss, serialize_identifier};
    ...
    fn parent_selector() {
        ...
        let parent = match parse(".bar, div .baz") {
            Ok(sel) => sel,
            Err(e) => {
                panic!("BUG: Test failed to parse parent selector: {:?}", e);
            },
        };
        let child = match parse("#foo &.bar") {
            Ok(sel) => sel,
            Err(e) => {
                panic!("BUG: Test failed to parse child selector: {:?}", e);
            },
        };
        let expected_result = match parse("#foo :is(.bar, div .baz).bar") {
            Ok(sel) => sel,
            Err(e) => {
                panic!("BUG: Test failed to parse expected result: {:?}", e);
            },
        };
        assert_eq!(child.replace_parent_selector(&parent), expected_result);

        let has_child = match parse("#foo:has(&.bar)") {
            Ok(sel) => sel,
            Err(e) => {
                panic!("BUG: Test failed to parse has_child selector: {:?}", e);
            },
        };
        let expected_has_result = match parse("#foo:has(:is(.bar, div .baz).bar)") {
            Ok(sel) => sel,
            Err(e) => {
                panic!("BUG: Test failed to parse expected has result: {:?}", e);
            },
        };
        assert_eq!(
        ...
        let child = match parse_relative_expected("#foo", ParseRelative::ForNesting, Some("& #foo"))
        {
            Ok(sel) => sel,
            Err(e) => panic!("BUG: Test failed to parse relative selector: {:?}", e),
        };
        let expected_foo_result = match parse(":is(.bar, div .baz) #foo") {
            Ok(sel) => sel,
            Err(e) => panic!("BUG: Test failed to parse expected foo result: {:?}", e),
        };
        assert_eq!(child.replace_parent_selector(&parent), expected_foo_result);

        let child_plus =
            match parse_relative_expected("+ #foo", ParseRelative::ForNesting, Some("& + #foo")) {
                Ok(sel) => sel,
                Err(e) => panic!("BUG: Test failed to parse relative plus selector: {:?}", e),
            };
        let expected_plus_result = match parse("& + #foo") {
            Ok(sel) => sel,
            Err(e) => panic!("BUG: Test failed to parse expected plus result: {:?}", e),
        };
        assert_eq!(child_plus, expected_plus_result);
    }

    #[test]
    fn test_pseudo_iter() {
        let list = match parse("q::before") {
            Ok(list) => list,
            Err(e) => panic!("BUG: Test failed to parse pseudo-element selector: {:?}", e),
        };
    ...
    #[test]
    fn test_universal() {
        let list = match parse_ns(
            "*|*::before",
            &DummyParser::default_with_namespace(DummyAtom::from("https://mozilla.org")),
        ) {
            Ok(list) => list,
            Err(e) => panic!("BUG: Test failed to parse universal selector: {:?}", e),
        };
        let selector = &list.slice()[0];
        assert!(selector.is_universal());
    }
    ...
    fn test_empty_pseudo_iter() {
        let list = match parse("::before") {
            Ok(list) => list,
            Err(e) => panic!("BUG: Test failed to parse empty pseudo selector: {:?}", e),
        };
        let selector = &list.slice()[0];
        assert!(selector.is_universal());
        let mut iter = selector.iter();
    ...
    fn test_parse_implicit_scope() {
        let parsed = match parse_relative_expected(".foo", ParseRelative::ForScope, None) {
            Ok(parsed) => parsed,
            Err(e) => panic!("BUG: Test failed to parse implicit scope selector: {:?}", e),
        };
        ...
        let scope_parsed =
            match parse_relative_expected(":scope .foo", ParseRelative::ForScope, None) {
                Ok(parsed) => parsed,
                Err(e) => panic!("BUG: Test failed to parse scope selector: {:?}", e),
            };
        let scope_expected = SelectorList::from_vec(vec![Selector::from_vec(
            vec![
                Component::Scope,
                Component::Combinator(Combinator::Descendant),
                Component::Class(DummyAtom::from("foo")),
        ...
        assert_eq!(scope_parsed, scope_expected);

        let child_parsed =
            match parse_relative_expected("> .foo", ParseRelative::ForScope, Some("> .foo")) {
                Ok(parsed) => parsed,
                Err(e) => panic!("BUG: Test failed to parse child scope selector: {:?}", e),
            };
        let child_expected = SelectorList::from_vec(vec![Selector::from_vec(
            vec![
                Component::ImplicitScope,
                Component::Combinator(Combinator::Child),
                Component::Class(DummyAtom::from("foo")),
        ...
        assert_eq!(child_parsed, child_expected);
