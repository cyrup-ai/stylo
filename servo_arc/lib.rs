...
impl<T: ?Sized> Arc<T> {
    ...
    unsafe fn drop_slow(&mut self) {
        ...
        let layout = unsafe { Layout::for_value(&*inner) };
        #[cfg(feature = "track_alloc_size")]
        let layout =
            unsafe { Layout::from_size_align_unchecked((*inner).alloc_size, layout.align()) };

        unsafe { std::ptr::drop_in_place(inner) };
        unsafe { alloc::dealloc(inner as *mut _, layout) };
    }
