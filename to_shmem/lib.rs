...
impl SharedMemoryBuilder {
    ...
    pub fn alloc_array<T>(&mut self, len: usize) -> *mut T {
        ...
        let total_size = match padded_size(size, align).checked_mul(len) {
            Some(size) => size,
            None => {
                panic!(
                    "BUG: Integer overflow in to_shmem allocation: size={} align={} len={}",
                    size, align, len
                );
            },
        };
        ...
        let layout = match Layout::from_size_align(total_size, align) {
            ...
            Err(e) => {
                panic!(
                    "BUG: Invalid layout parameters in to_shmem allocation: size={} align={} error={:?}",
                    total_size, align, e
                );
            },
        };
    ...
    pub fn alloc<T>(&mut self, layout: Layout) -> *mut T {
        ...
        let start = match self.index.checked_add(padding) {
            Some(start) => start,
            None => {
                panic!(
                    "BUG: Integer overflow when adding padding to to_shmem buffer index: index={} padding={}",
                    self.index, padding
                );
            },
        };
        ...
        let end = match start.checked_add(layout.size()) {
            ...
            None => {
                panic!(
                    "BUG: Integer overflow when adding layout size to to_shmem buffer: start={} size={}",
                    start,
                    layout.size()
                );
            },
...
{
    let dest = unsafe { slice::from_raw_parts_mut(dest, src.len()) };
...
impl<T: ToShmem, const N: usize> ToShmem for smallvec::SmallVec<T, N> {
    fn to_shmem(&self, builder: &mut SharedMemoryBuilder) -> Result<Self> {
        let dest_vec = unsafe {
            if self.spilled() {
...
impl<T: ToShmem> ToShmem for thin_vec::ThinVec<T> {
    fn to_shmem(&self, builder: &mut SharedMemoryBuilder) -> Result<Self> {
        ...
        let total_size = header_size + header_padding + padded_size(item_size, item_align) * len;
        let layout = match Layout::from_size_align(total_size, item_align) {
            Ok(layout) => layout,
            Err(e) => {
                panic!(
                    "BUG: Invalid layout parameters for ThinVec in to_shmem: size={} align={} error={:?}",
                    total_size, item_align, e
                );
            },
        };
