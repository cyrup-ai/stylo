...
impl SharedRwLock {
    ...
    pub fn write(&self) -> SharedRwLockWriteGuard<'_> {
        match self.cell.as_ref() {
            Some(cell) => SharedRwLockWriteGuard(cell.borrow_mut()),
            None => {
                panic!("BUG: SharedRwLock write() called on uninitialized cell - this indicates a serious threading or initialization issue");
            },
        }
    }
}
...
pub trait ToCssWithGuard {
    ...
    fn to_css_string(&self, guard: &SharedRwLockReadGuard) -> CssString {
        ...
        match self.to_css(guard, &mut s) {
            Ok(_) => s,
            Err(e) => {
                eprintln!("Warning: Failed to serialize CSS to string: {}", e);
                CssString::new()
            },
        }
    }
