...
impl IntermediateComputedKeyframe {
    ...
    fn generate_for_keyframes(
        ...
        debug_assert!({
            match intermediate_steps.first() {
                Some(first) => first.start_percentage == 0.,
                None => {
                    eprintln!("BUG: Animation intermediate_steps is empty - should have at least first step at 0%");
                    false
                },
            }
        });
        debug_assert!({
            match intermediate_steps.last() {
                Some(last) => last.start_percentage == 1.,
                None => {
                    eprintln!("BUG: Animation intermediate_steps is empty - should have at least last step at 100%");
                    false
                },
            }
        });
