...
#[derive(
    Clone, MallocSizeOf, PartialEq, ToAnimatedValue, ToComputedValue, ToResolvedValue, ToShmem,
)]
#[allow(missing_docs)]
#[repr(C)]
...
impl<Color, Percentage> std::fmt::Debug for GenericColorMix<Color, Percentage>
where
    Percentage: std::fmt::Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GenericColorMix")
            .field("interpolation", &self.interpolation)
            .field("left", &"<Color>") // Avoid recursion by not debugging the Color field
            .field("left_percentage", &self.left_percentage)
            .field("right", &"<Color>") // Avoid recursion by not debugging the Color field
            .field("right_percentage", &self.right_percentage)
