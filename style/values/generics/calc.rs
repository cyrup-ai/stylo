...
#[repr(u8)]
#[derive(
    Clone, Deserialize, MallocSizeOf, PartialEq, Serialize, ToAnimatedZero, ToResolvedValue, ToShmem,
)]
...
impl<L> std::fmt::Debug for GenericCalcNode<L>
where
    L: std::fmt::Debug,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Negate(_) => f.debug_tuple("Negate").field(&"<CalcNode>").finish(),
            Self::Invert(_) => f.debug_tuple("Invert").field(&"<CalcNode>").finish(),
            Self::Sum(_) => f.debug_tuple("Sum").field(&"<CalcNodes>").finish(),
            Self::Product(_) => f.debug_tuple("Product").field(&"<CalcNodes>").finish(),
            Self::MinMax(_, op) => f
                .debug_tuple("MinMax")
                .field(&"<CalcNodes>")
                .field(op)
            ...
            Self::Clamp { .. } => f
                .debug_struct("Clamp")
                .field("min", &"<CalcNode>")
                .field("center", &"<CalcNode>")
                .field("max", &"<CalcNode>")
                .finish(),
            Self::Round { strategy, .. } => f
                .debug_struct("Round")
                .field("strategy", strategy)
                .field("value", &"<CalcNode>")
                .field("step", &"<CalcNode>")
                .finish(),
            Self::ModRem { op, .. } => f
                .debug_struct("ModRem")
                .field("dividend", &"<CalcNode>")
                .field("divisor", &"<CalcNode>")
                .field("op", op)
                .finish(),
            Self::Hypot(_) => f.debug_tuple("Hypot").field(&"<CalcNodes>").finish(),
            Self::Abs(_) => f.debug_tuple("Abs").field(&"<CalcNode>").finish(),
            Self::Sign(_) => f.debug_tuple("Sign").field(&"<CalcNode>").finish(),
            Self::Anchor(_) => f.debug_tuple("Anchor").field(&"<AnchorFunction>").finish(),
            Self::AnchorSize(_) => f
                .debug_tuple("AnchorSize")
                .field(&"<AnchorSizeFunction>")
                .finish(),
        }
    }
}
