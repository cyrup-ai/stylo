...
impl NoCalcLength {
    ...
    pub(crate) fn map(&self, mut op: impl FnMut(f32) -> f32) -> Self {
        ...
        match self {
            Absolute(one) => Absolute(one.map(op)),
            FontRelative(one) => FontRelative(one.map(op)),
            ViewportPercentage(one) => ViewportPercentage(one.map(op)),
            ContainerRelative(one) => ContainerRelative(one.map(op)),
            ServoCharacterWidth(one) => {
