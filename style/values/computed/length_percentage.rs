...
#[cfg(feature = "gecko")]
use crate::{
    gecko_bindings::structs::AnchorPosOffsetResolutionParams, logical_geometry::PhysicalSide,
};
    ...
    fn from_computed_value(computed: &CalcLengthPercentage) -> Self {
        ...
        specified::CalcLengthPercentage {
            ...
            node: computed.node.map_leaves(|l| match l {
                CalcLengthPercentageLeaf::Length(l) => Leaf::Length(NoCalcLength::from_px(l.px())),
                CalcLengthPercentageLeaf::Percentage(p) => Leaf::Percentage(p.0),
