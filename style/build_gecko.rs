...
// Mock mozbuild when not available
mod mozbuild_mock {
    use std::path::PathBuf;

    pub static TOPSRCDIR: std::sync::LazyLock<PathBuf> =
        std::sync::LazyLock::new(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")));

    pub static TOPOBJDIR: std::sync::LazyLock<PathBuf> =
        std::sync::LazyLock::new(|| std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")));

    pub mod config {
        pub static BINDGEN_SYSTEM_FLAGS: &[&str] = &[];
        pub static NSPR_CFLAGS: &[&str] = &[];
        pub static MOZ_PIXMAN_CFLAGS: &[&str] = &[];
        pub static MOZ_ICU_CFLAGS: &[&str] = &[];
    }
}

// Always use mock since the real mozbuild crate is not available
use mozbuild_mock as mozbuild;
...
lazy_static! {
    ...
    static ref BINDGEN_FLAGS: Vec<String> = {
        mozbuild::config::BINDGEN_SYSTEM_FLAGS
            .iter()
            .chain(mozbuild::config::NSPR_CFLAGS.iter())
            .chain(mozbuild::config::MOZ_PIXMAN_CFLAGS.iter())
            .chain(mozbuild::config::MOZ_ICU_CFLAGS.iter())
            .map(|s| s.to_string())
            .collect()
    };
