...
impl<'scope, T: Send> ScopedTLS<'scope, T> {
    ...
    pub fn current_thread_index(&self) -> usize {
        self.pool.map_or(0, |p| {
            p.current_thread_index().unwrap_or_else(|| {
                // Fallback to thread 0 if we can't determine the current thread index
                eprintln!("Warning: Unable to determine current thread index, defaulting to 0");
                0
            })
        })
    }
    ...
    pub fn ensure<F: FnOnce(&mut Option<T>)>(&self, f: F) -> RefMut<'_, T> {
        ...
        RefMut::map(opt, |x| {
            x.as_mut().unwrap_or_else(|| {
                // This should never happen due to the initialization above
                panic!("BUG: ScopedTLS ensure() failed to initialize value after calling initializer function");
            })
        })
    }
