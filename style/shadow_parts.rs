...
mod tests {
    ...
    fn parse_valid_mapping_list() {
        let input = "foo: bar, totally-invalid-mapping,,";
        let mut mappings = parse_mapping_list(input);
        ...
        // "totally-invalid-mapping" is a valid single token mapping (maps to itself per CSS spec)
        assert_eq!(
            mappings.next(),
            Some(("totally-invalid-mapping", "totally-invalid-mapping")),
            "Single token should map to itself"
        );

        // "" is not valid (empty/whitespace only), so no more mappings should exist
